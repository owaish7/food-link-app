...
import img from '../../assets/team-3-800x800.jpg'
...
export default function RestaurantProfilePage() {
    ...
    useEffect(() => {
        const fetchUserInfo = async () => {
            try {
                const response = await axios.get(`${API_URL}/restaurant/profile/${user._id}`);
                                            ...
                                            <div className="flex flex-wrap justify-center">
                                                <div className="w-full lg:w-9/12 px-4">
                                                    <p className={`mb-4 text-lg leading-relaxed ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                                                        This restaurant has fulfilled {userInfo.fulfilledOrders || 0} orders.
                                                        {userInfo.totalOrders > 0 &&
                                                            ` ${Math.round((userInfo.fulfilledOrders / userInfo.totalOrders) * 100)}% of the orders have been fulfilled.`
                                                        }
                                                        {userInfo.totalOrders > 0 &&
                                                            ` ${Math.round((userInfo.cancelledOrders / userInfo.totalOrders) * 100)}% of the orders have been cancelled.`
                                                        }
                                                        FoodLink is proud to have partnered with such a reputable establishment. With their exceptional service and dedication to quality, they have become a cornerstone of our community. We highly recommend dining at {userInfo.username} for an unforgettable culinary experience.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
